...
export default {
    ...
    async execute({ client, message, interaction, args }) {
        ...
        const success = await client.giveaways.resume(messageId);

        if (success) {
            const successMsg = `${client.config.emojis.success} Giveaway resumed successfully!`;
            ...
            const errorMsg = `${client.config.emojis.error} Failed to resume giveaway. Ensure the ID is correct and it's actually paused.`;
            if (isSlash) return interaction.reply({ content: errorMsg, flags: KyraUI.getFlags(true) });
            return message.reply({ content: errorMsg }).then(msg => setTimeout(() => msg.delete().catch(() => { }), 5000)).catch(() => { });
        }
    }
