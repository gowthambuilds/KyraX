...
export default {
    name: 'vdeafen',
    description: 'Server deafens a member in a voice channel',
    aliases: ['voicedeafen'],
    slash: true,
    options: [
        { name: 'user', description: 'The user to voice deafen', type: 6, required: true },
        { name: 'reason', description: 'Reason for the voice deafen', type: 3, required: false }
    ],
    async execute({ client, message, interaction, args }) {
        ...
        if (!targetMember) {
            return KyraUI.sendUsage({ client, message, interaction }, isSlash ? '/vdeafen user: <user> [reason: reason]' : `${client.prefix}vdeafen <user> [reason]`);
        }
        ...
        if (targetMember.voice.serverDeaf) {
            const errorContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.error} This member is already server deafened.`);
            return isSlash ? interaction.editReply({ components: errorContainer }) : message.reply({ components: errorContainer, flags: KyraUI.getFlags() });
        }

        try {
            await targetMember.voice.setDeaf(true, `Moderator: ${executor.user.tag} | Reason: ${reason}`);

            const successContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.success} **${targetMember.user.username}** has been server deafened.\n${client.config.emojis.dot} **Reason:** ${reason}`);
            ...
            client.logger.error('VDeafen', 'Failed to voice deafen member', error);
            const errorContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.error} An error occurred while trying to voice deafen this member.`);
            return isSlash ? interaction.editReply({ components: errorContainer }) : message.reply({ components: errorContainer, flags: KyraUI.getFlags() });
        }
    }
};
