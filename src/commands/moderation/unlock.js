...
export default {
    ...
    async execute({ client, message, interaction, args }) {
        ...
        try {
            ...
            const successContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.success} ${targetChannel} has been **unlocked**.`);
            ...
            client.logger.error('Unlock', 'Failed to unlock channel', error);
            const errorContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.error} An error occurred while trying to unlock the channel.`);
            return isSlash ? interaction.editReply({ components: errorContainer }) : message.reply({ components: errorContainer, flags: KyraUI.getFlags() });
        }
    }
};
