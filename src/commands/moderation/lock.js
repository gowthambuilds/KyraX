...
export default {
    ...
    async execute({ client, message, interaction, args }) {
        ...
        try {
            ...
            const successContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.success} ${targetChannel} has been **locked**.`);
            ...
            client.logger.error('Lock', 'Failed to lock channel', error);
            const errorContainer = KyraUI.buildSimpleMessage(`${client.config.emojis.error} An error occurred while trying to lock the channel.`);
            return isSlash ? interaction.editReply({ components: errorContainer }) : message.reply({ components: errorContainer, flags: KyraUI.getFlags() });
        }
    }
};
